...
export default function VentasVentanas() {
  ...
  const calcularUnitario = () => {
    ...
    const detalle = (tipoSeleccionado.materiales || []).map((mat) => {
      ...
      const longitudTotal = longitudNeta * cantMat; // como Excel
    ...
    const totalUnitario = detalle.reduce((acc, it) => acc + it.subtotal, 0);

    return { detalle, totalUnitario, ctx };
  };
  ...
  const agregarAlCarrito = () => {
    ...
    try {
      ...
      const { detalle, totalUnitario, ctx } = calcularUnitario();
      ...
      const item = {
        ...
        cantidad: cant,
        totalUnitario,
        total: totalUnitario * cant,
        detalle
      };

      setCarrito((prev) => [...prev, item]);

      // limpiar para el siguiente agregado
      setAncho("");
      setAlto("");
      setX("0");
      setY("0");
  ...
  const total = useMemo(
    () => carrito.reduce((acc, it) => acc + Number(it.total || 0), 0),
    [carrito]
  );
    ...
    factura.items.forEach((i) => {
      ...
      const vars =
        (Number(i.x) || Number(i.y) || Number(i.z))
          ...
          : "";
      const nombre = base + vars;

      docpdf.text(nombre, M_LEFT + 2, yPos, { maxWidth: 78 });
      docpdf.text(String(i.cantidad), 86, yPos, { align: "right" });
      docpdf.text(`C$ ${Number(i.totalUnitario).toFixed(2)}`, 110, yPos, { align: "right" });
      docpdf.text(`C$ ${Number(i.total).toFixed(2)}`, 134, yPos, { align: "right" });
                  ...
                  {carrito.map((it) => (
                    <div key={it.id} className="border rounded p-2">
                      ...
                      <div className="d-flex justify-content-between mt-2">
                        <span className="text-muted">Unitario</span>
                        <span className="fw-bold">C$ {it.totalUnitario.toFixed(2)}</span>
                      </div>
                      <div className="d-flex justify-content-between">
                        <span className="text-muted">Subtotal</span>
                        <span className="fw-bold">C$ {it.total.toFixed(2)}</span>
                      </div>
                    </div>
                  ))}
                </div>
